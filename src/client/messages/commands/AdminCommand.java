...
public class AdminCommand
{
    ...
    public static class LevelUp extends CommandExecute
    {
        ...
        public int execute(MapleClient c, String[] splitted)
        {
            if (c.getPlayer().getLevel() < 200) {
                c.getPlayer().gainExp(1000000000, true, false, true);
            }
    ...
    public static class GiveMaplePoint extends CommandExecute
    {
        ...
        public int execute(MapleClient c, String[] splitted)
        {
            if (3 != splitted.length) {
                c.getPlayer().dropMessage(6, "格式錯誤： !givemaplepoint <玩家名稱> <數量>");

                return 0;
            }

            if (splitted[1].equals("everyone")) {
                for (ChannelServer cserv : ChannelServer.getAllInstances()) {
                    for (MapleCharacter mch : cserv.getPlayerStorage().getAllCharacters()) {
                        mch.modifyCSPoints(2, Integer.parseInt(splitted[2]), true);
                    }
                }
            } else {
                MapleCharacter mch = c.getChannelServer().getPlayerStorage().getCharacterByName(splitted[1]);

                if (null == mch) {
                    c.getPlayer().dropMessage(6, "玩家「" + splitted[1] + "」不存在");
                } else {
                    mch.modifyCSPoints(2, Integer.parseInt(splitted[2]), true);
                }
            }
    ...
    public static class GiveMeso extends CommandExecute
    {
        ...
        {
            if (3 != splitted.length) {
                c.getPlayer().dropMessage(6, "格式錯誤： !givemeso <玩家名稱> <數量>");
            ...
            if (splitted[1].equals("everyone")) {
                for (ChannelServer cserv : ChannelServer.getAllInstances()) {
                    for (MapleCharacter mch : cserv.getPlayerStorage().getAllCharacters()) {
                        mch.gainMeso(Integer.parseInt(splitted[2]), true);
                    }
                }
            } else {
                MapleCharacter mch = c.getChannelServer().getPlayerStorage().getCharacterByName(splitted[1]);
    ...
    public static class SendNote extends CommandExecute
    {
        ...
        public int execute(MapleClient c, String[] splitted)
        {
            if (splitted.length < 1) {
                c.getPlayer().dropMessage(6, "格式錯誤： !sendnote <訊息>");
    ...
    public static class DC extends CommandExecute
    {
        ...
        public int execute(MapleClient c, String[] splitted)
        {
            if (splitted.length != 2) {
                c.getPlayer().dropMessage(6, "格式錯誤： !dc <玩家名稱>");
    ...
    public static class Kill extends CommandExecute
    {
        ...
        {
            if (splitted.length < 2) {
                c.getPlayer().dropMessage(6, "格式錯誤： !kill <玩家名稱> [<玩家名稱> ...]");
    ...
    public static class SaveAll extends CommandExecute
    {
        ...
        public int execute(MapleClient c, String[] splitted)
        {
            ...
            c.getPlayer().dropMessage("儲存完畢，共計 " + count + " 位玩家");
    ...
    public static class Shutdown extends CommandExecute
    {
        ...
        {
            if (t != null && t.isAlive()) {
                c.getPlayer().dropMessage(6, "伺服器已在關閉中");
            } else {
     ...
     * 於指定分鐘數後關閉伺服器
     */
    public static class ShutdownIn extends CommandExecute
    {
        ...
        public int execute(MapleClient c, String[] splitted)
        {
            if (2 != splitted.length) {
                c.getPlayer().dropMessage(6, "格式錯誤： !shutdownin <分鐘>");

                return 0;
            }

            if (ts != null || (t != null && t.isAlive())) {
                c.getPlayer().dropMessage(6, "伺服器已有關閉排程，請勿重複執行此指令");
            } else {
                ...
                ts = EventTimer.getInstance().register(new Runnable() {
                    ...
                    public void run()
                    {
                        ...
                        World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(0, " 伺服器即將在 " + minutesLeft + " 分鐘後關閉，請盡速關閉精靈商人並下線").getBytes());
