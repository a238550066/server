...
public final class MapleMap {
    ...
    private void dropFromMonster(final MapleCharacter chr, final MapleMonster mob)
    {
        ...
        final MapleItemInformationProvider itemInfo = MapleItemInformationProvider.getInstance();
        ...
        final int mobXPos = mob.getPosition().x;
        final int channelMesoRate = ChannelServer.getInstance(channel).getMesoRate();
        ...
        for (final MonsterDropEntry de : dropEntry) {
            ...
            // 很遺憾，未中獎，下次加油
            if (Randomizer.nextInt(1000000) >= (int) (de.chance * channelDropRate * chr.getDropMod() * (chr.getStat().dropBuff / 100.0) * (showdown / 100.0))) {
                continue;
            }

            pos.x = this.calcDropXPos(mobXPos, dropType, d);
            ...
            if (0 == de.itemId) { // meso
                ...
                if (GameConstants.getInventoryType(de.itemId) == MapleInventoryType.EQUIP) {
                    inventoryDrop = itemInfo.randomizeStats((Equip) itemInfo.getEquipById(de.itemId));
                } else {
                    inventoryDrop = new Item(de.itemId, (byte) 0, (short) de.getQuantity(), (byte) 0);
                }

                spawnMobDrop(inventoryDrop, calcDropPos(pos, mob.getPosition()), mob, chr, dropType, de.questId);
            }
        ...
        // 如果是 boss 且 hp 顯示類行為 0 ，則以頻道點
        // 數倍率為基礎加乘 20 倍，否則即為頻道點數倍率
        final int cashRate = (mob.getStats().isBoss() && mob.getStats().getHPDisplayType() == 0 ? 20 : 1) * channelCashRate;

        // 如果不是 boss，則以 怪物經驗值/1000 加上
        // 怪物血量/10000 作為調整值，否則為 0
        final int cashModifier = (int) ((mob.getStats().isBoss() ? 0 : (mob.getMobExp() / 1000 + mob.getMobMaxHp() / 10000)));
        ...
        for (final MonsterGlobalDropEntry de : globalEntry) {
            ...
            // 是楓葉點數
            if (0 == de.itemId) {
                chr.modifyCSPoints(2, (int) ((Randomizer.nextInt(cashRate) + cashRate + cashModifier) * (chr.getStat().cashBuff / 100.0) * chr.getCashMod()), true);
            ...
            // 終於可以掉寶了
            spawnMobDrop(inventoryDrop, calcDropPos(pos, mob.getPosition()), mob, chr, de.dropType, de.questId);
     ...
     * 計算物品掉落 X 軸位置
     *
     * @param mobXPos mob x position
     * @param dropType item drop type
     * @param counter current drop sequence
     * @return item drop position
     */
    private int calcDropXPos(final int mobXPos, final byte dropType, final byte counter)
    {
        if (dropType == 3) {
            return (mobXPos + ((counter % 2 == 0) ? (40 * (counter + 1) / 2) : -(40 * (counter / 2))));
        }

        return (mobXPos + ((counter % 2 == 0) ? (25 * (counter + 1) / 2) : -(25 * (counter / 2))));
    }
